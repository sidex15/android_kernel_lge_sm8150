...
#if IS_ENABLED(CONFIG_LGE_DUAL_SCREEN)
	pr_debug("process_hpd_high done: rc=%d, will_notify=%d\n", rc, !rc);
#endif
	if (!rc)
...
#else
#if IS_ENABLED(CONFIG_LGE_DUAL_SCREEN)
	pr_debug("hpd_irq:%d, hpd_high:%d, power_on:%d, is_connected:%d, process_hpd_connect:%d\n",
			dp->hpd->hpd_irq, dp->hpd->hpd_high,
...
#if IS_ENABLED(CONFIG_LGE_DUAL_SCREEN)
		pr_debug("attention_cb: ignored (hpd_high but already connected)\n");
#else
		pr_debug("ignored\n");
#endif
