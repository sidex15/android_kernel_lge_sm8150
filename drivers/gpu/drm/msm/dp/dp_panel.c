 ...
 * Reading its EDID over the AUX channel through the on-board iCE40 crossbar is
 * unreliable/orientation-sensitive on flash, and by the time routing is correct
 * the DP core has already latched a failsafe mode. Mirror the DS1 approach:
 * skip the AUX EDID read and hand the DP core a known-good EDID.
 *
 * Capture the real bytes from a native V50s/G8x with the DS attached:
 *   cat /sys/class/drm/card0-DP-1/edid | xxd -i
 * and paste the first 128 bytes below (block 0). Until then this is a
 * synthesized 1080x2340@60 CVT-RB block which the panel will accept as a mode.
 */
static bool ds2_hardcoded_edid = true;
module_param(ds2_hardcoded_edid, bool, 0644);
// v50s/g8x dual screen panel edid
u8 ds2_edid[128] = {
